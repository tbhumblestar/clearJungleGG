...
## 3. 개발 방식: API Spec 기반 TDD

백엔드 API는 다음 순서로 개발한다.

```
1. API Spec 정의 (2_API_SPEC.md)
   → 엔드포인트, 요청/응답 JSON 스키마를 먼저 확정

2. 테스트 코드 작성
   → API Spec에 명시된 요청을 보내고, 명시된 응답이 오는지 검증하는 테스트
   → 이 시점에서 테스트는 실패한다 (RED)

3. API 구현
   → 테스트를 통과시키기 위한 최소한의 구현 (GREEN)

4. 리팩토링
   → 테스트가 통과하는 상태를 유지하면서 코드 정리 (REFACTOR)
```

API Spec이 곧 계약(Contract)이다. 프론트엔드는 이 스펙을 믿고 mock 데이터를 만들었고, 백엔드는 이 스펙을 만족시키는 것이 목표다.
